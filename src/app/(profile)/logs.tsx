...
export default function LogsScreen() {
  ...
  return (
    <Screen className="px-4" style={{ width: '100%', maxWidth: 900, alignSelf: 'center' }}>
      <PageHeader title={t('dfLogsTitle')} subtitle={t('dfLogsSubtitle', { count: filtered.length })} onBack={() => router.back()} className="px-0" right={<Button size="sm" variant="outline" onPress={() => void refreshLogs()} disabled={loading || !gameKey}><Icon as={RefreshCw} size={15} /><Text>{loading ? t('dfLogsRefreshing') : t('refresh')}</Text></Button>} />
      ...
      <Tabs value={kind} onValueChange={(value) => setKind(value as typeof kind)} className="mb-3"><TabsList><TabsTrigger value="story"><Text variant="small">{t('dfLogsStoryTab')}</Text></TabsTrigger><TabsTrigger value="private"><Text variant="small">{t('dfLogsPrivateTab')}</Text></TabsTrigger></TabsList></Tabs>
      <View className="mb-3 flex-row items-center gap-2 rounded-xl border border-border bg-card px-3"><Icon as={Search} size={17} className="text-muted-foreground" /><Input value={query} onChangeText={setQuery} placeholder={t('dfLogsSearchPlaceholder')} className="flex-1 rounded-none border-0 bg-transparent px-0 shadow-none" /></View>
      <FlatList
        data={filtered}
